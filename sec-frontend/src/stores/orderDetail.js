...
export const useOrderDetailStore = defineStore('orderDetail', {
  ...
  actions: {
    ...
    async fetchList(params = {}) {
      this.loading = true
      try {
        const res = await orderDetailApi.list({ page: this.page, size: this.size, ...params })
    ...
    // 新增
    async create(data) {
      const res = await orderDetailApi.create(data)
      return res
    },
    // 修改
    ...
    // 删除
    async remove(id) {
      const res = await orderDetailApi.delete(id)
      return res
    }
  },
  persist: true
