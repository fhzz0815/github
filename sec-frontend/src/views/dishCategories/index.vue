<template>
  <div class="dishCategories-page">
        <!-- 搜索区域 -->
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm">
        <el-form-item label="关键字">
          <el-input v-model="queryForm.keyword" placeholder="请输入分类名称" clearable style="width: 220px" @keyup.enter="handleSearch" />
        </el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      ...
      <el-table :data="dishCategoryStore.list" v-loading="dishCategoryStore.loading" border stripe style="width: 100%">
        <el-table-column type="index" label="序号" width="60" align="center" />
        <el-table-column prop="storeId" label="门店ID" width="120" show-overflow-tooltip />
        <el-table-column prop="categoryName" label="分类名称" width="120" show-overflow-tooltip />
      ...
      <div class="pagination">
        <el-pagination
          v-model:current-page="dishCategoryStore.page"
          v-model:page-size="dishCategoryStore.size"
          :total="dishCategoryStore.total"
...
import { useDishCategoryStore } from '@/stores/dishCategory'
import dishCategoryApi from '@/api/dishCategory'

const dishCategoryStore = useDishCategoryStore()
...
const handleSearch = () => {
  dishCategoryStore.page = 1
  dishCategoryStore.fetchList(buildParams())
}
const handleReset = () => {
  Object.keys(queryForm).forEach((k) => { queryForm[k] = k === 'dateRange' ? null : '' })
  dishCategoryStore.page = 1
  dishCategoryStore.fetchList()
}
// 翻页时保留搜索条件
const onPageChange = () => {
  dishCategoryStore.fetchList(buildParams())
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除这条记录吗？', '提示', { type: 'warning' })
    .then(async () => {
      await dishCategoryApi.delete(row.id)
...
const handleSubmit = async () => {
  if (formData.id) {
    await dishCategoryApi.update(formData.id, formData)
  } else {
    await dishCategoryApi.create(formData)
  }
...
<style scoped>
.dishCategories-page { padding: 16px; }
