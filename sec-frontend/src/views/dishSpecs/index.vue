<template>
  <div class="dishSpecs-page">
        <!-- 搜索区域 -->
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryForm">
        <el-form-item label="关键字">
          <el-input v-model="queryForm.keyword" placeholder="请输入规格名称" clearable style="width: 220px" @keyup.enter="handleSearch" />
        </el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      ...
      <el-table :data="dishSpecStore.list" v-loading="dishSpecStore.loading" border stripe style="width: 100%">
        ...
        <el-table-column prop="priceDelta" label="加价金额(元)" width="160" show-overflow-tooltip />
      ...
      <div class="pagination">
        <el-pagination
          v-model:current-page="dishSpecStore.page"
          v-model:page-size="dishSpecStore.size"
          :total="dishSpecStore.total"
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="700px">
      <el-form :model="formData" label-width="120px">
        ...
        <el-form-item label="规格名称">
          <el-input v-model="formData.specName" placeholder="请输入规格名称" />
        </el-form-item>
        <el-form-item label="加价金额(元)">
          <el-input-number v-model="formData.priceDelta" placeholder="请输入加价金额(元)" style="width:100%" />
        </el-form-item>
...
import { useDishSpecStore } from '@/stores/dishSpec'
import dishSpecApi from '@/api/dishSpec'

const dishSpecStore = useDishSpecStore()
...
const handleSearch = () => {
  dishSpecStore.page = 1
  dishSpecStore.fetchList(buildParams())
}
const handleReset = () => {
  Object.keys(queryForm).forEach((k) => { queryForm[k] = k === 'dateRange' ? null : '' })
  dishSpecStore.page = 1
  dishSpecStore.fetchList()
}
// 翻页时保留搜索条件
const onPageChange = () => {
  dishSpecStore.fetchList(buildParams())
...
const handleAdd = () => {
  dialogTitle.value = '新增菜品规格'
  formData.id = null
  formData.storeId = ''
  formData.specName = ''
  formData.priceDelta = null
  formData.sort = ''
  formData.status = ''
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除这条记录吗？', '提示', { type: 'warning' })
    .then(async () => {
      await dishSpecApi.delete(row.id)
...
const handleSubmit = async () => {
  if (formData.id) {
    await dishSpecApi.update(formData.id, formData)
  } else {
    await dishSpecApi.create(formData)
  }
