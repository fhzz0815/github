<template>
  <div class="stockCheckIngredients-page">
    <!-- 搜索区域 -->
    ...
    <el-card class="table-card" shadow="never">
      ...
      <el-table :data="stockCheckIngredientStore.list" v-loading="stockCheckIngredientStore.loading" border stripe style="width: 100%">
      ...
      <div class="pagination">
        <el-pagination
          v-model:current-page="stockCheckIngredientStore.page"
          v-model:page-size="stockCheckIngredientStore.size"
          :total="stockCheckIngredientStore.total"
          :page-sizes="[10, 20, 50, 100]"
          layout="total, sizes, prev, pager, next, jumper"
          @size-change="stockCheckIngredientStore.fetchList"
          @current-change="stockCheckIngredientStore.fetchList"
        />
      </div>
    </el-card>
...
import { useStockCheckIngredientStore } from '@/stores/stockCheckIngredient'
import stockCheckIngredientApi from '@/api/stockCheckIngredient'

const stockCheckIngredientStore = useStockCheckIngredientStore()

const queryForm = reactive({ keyword: '' })

const dialogVisible = ref(false)
const dialogTitle = ref('新增原料盘点')
...
const handleSearch = () => {
  stockCheckIngredientStore.page = 1
  stockCheckIngredientStore.fetchList()
}
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除这条记录吗？', '提示', { type: 'warning' })
    .then(async () => {
      await stockCheckIngredientApi.delete(row.id)
...
const handleSubmit = async () => {
  if (formData.id) {
    await stockCheckIngredientApi.update(formData.id, formData)
  } else {
    await stockCheckIngredientApi.create(formData)
  }
...
<style scoped>
.stockCheckIngredients-page { padding: 16px; }
