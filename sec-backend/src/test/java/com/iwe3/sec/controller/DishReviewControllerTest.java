...
/**
 * Unit test for DishReview Controller
 */
...
class DishReviewControllerTest {
    ...
    private DishReviewController controller;

    @BeforeEach
    void setUp() {
        controller = new DishReviewController(dishReviewService);
    }
    ...
    void testList_ShouldReturnPageResult() {
        ...
        List<DishReviewEntity> entityList = Arrays.asList(entity1);
        PageResult<DishReviewEntity> pageResult = PageResult.of(1L, 1, entityList);
        when(dishReviewService.list(any(), anyInt(), anyInt())).thenReturn(pageResult);

        Result<PageResult<DishReviewEntity>> result = controller.list(new DishReviewEntity(), 1, 10);
        ...
        verify(dishReviewService).list(any(), eq(1), eq(10));
    }
    ...
    void testGetById_WhenExists_ShouldReturnEntity() {
        ...
        when(dishReviewService.getById(1L)).thenReturn(mockEntity);

        Result<DishReviewEntity> result = controller.getById(1L);
    ...
    void testGetById_WhenNotExists_ShouldReturnNullData() {
        when(dishReviewService.getById(999L)).thenReturn(null);

        Result<DishReviewEntity> result = controller.getById(999L);
    ...
    void testAdd_ShouldCallService() {
        ...
        assertNotNull(result);
        assertEquals(0, result.getCode());
        verify(dishReviewService).add(entity);
    }
    ...
    void testUpdate_ShouldSetIdAndCallService() {
        ...
        verify(dishReviewService).update(entity);
    }
    ...
    void testDelete_ShouldCallService() {
        ...
        assertNotNull(result);
        assertEquals(0, result.getCode());
        verify(dishReviewService).remove(1L);
    }
}