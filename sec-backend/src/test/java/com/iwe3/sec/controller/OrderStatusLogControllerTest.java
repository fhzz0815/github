...
/**
 * Unit test for OrderStatusLog Controller
 */
...
class OrderStatusLogControllerTest {
    ...
    private OrderStatusLogController controller;

    @BeforeEach
    void setUp() {
        controller = new OrderStatusLogController(orderStatusLogService);
    }
    ...
    void testList_ShouldReturnPageResult() {
        ...
        List<OrderStatusLogEntity> entityList = Arrays.asList(entity1);
        PageResult<OrderStatusLogEntity> pageResult = PageResult.of(1L, 1, entityList);
        when(orderStatusLogService.list(any(), anyInt(), anyInt())).thenReturn(pageResult);

        Result<PageResult<OrderStatusLogEntity>> result = controller.list(new OrderStatusLogEntity(), 1, 10);
        ...
        verify(orderStatusLogService).list(any(), eq(1), eq(10));
    }
    ...
    void testGetById_WhenExists_ShouldReturnEntity() {
        ...
        when(orderStatusLogService.getById(1L)).thenReturn(mockEntity);

        Result<OrderStatusLogEntity> result = controller.getById(1L);
    ...
    void testGetById_WhenNotExists_ShouldReturnNullData() {
        when(orderStatusLogService.getById(999L)).thenReturn(null);

        Result<OrderStatusLogEntity> result = controller.getById(999L);
    ...
    void testAdd_ShouldCallService() {
        ...
        assertNotNull(result);
        assertEquals(0, result.getCode());
        verify(orderStatusLogService).add(entity);
    }
    ...
    void testUpdate_ShouldSetIdAndCallService() {
        ...
        verify(orderStatusLogService).update(entity);
    }
    ...
    void testDelete_ShouldCallService() {
        ...
        assertNotNull(result);
        assertEquals(0, result.getCode());
        verify(orderStatusLogService).remove(1L);
    }
}