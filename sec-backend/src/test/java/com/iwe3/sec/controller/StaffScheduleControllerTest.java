...
/**
 * Unit test for StaffSchedule Controller
 */
@ExtendWith(MockitoExtension.class)
class StaffScheduleControllerTest {

    @Mock
    private IStaffScheduleService staffScheduleService;

    private StaffScheduleController controller;

    @BeforeEach
    void setUp() {
        controller = new StaffScheduleController(staffScheduleService);
    }
    ...
    void testList_ShouldReturnPageResult() {
        ...
        List<StaffScheduleEntity> entityList = Arrays.asList(entity1);
        PageResult<StaffScheduleEntity> pageResult = PageResult.of(1L, 1, entityList);
        when(staffScheduleService.list(any(), anyInt(), anyInt())).thenReturn(pageResult);

        Result<PageResult<StaffScheduleEntity>> result = controller.list(new StaffScheduleEntity(), 1, 10);
        ...
        verify(staffScheduleService).list(any(), eq(1), eq(10));
    }
    ...
    void testGetById_WhenExists_ShouldReturnEntity() {
        ...
        when(staffScheduleService.getById(1L)).thenReturn(mockEntity);

        Result<StaffScheduleEntity> result = controller.getById(1L);
    ...
    void testGetById_WhenNotExists_ShouldReturnNullData() {
        when(staffScheduleService.getById(999L)).thenReturn(null);

        Result<StaffScheduleEntity> result = controller.getById(999L);
    ...
    void testAdd_ShouldCallService() {
        ...
        assertNotNull(result);
        assertEquals(0, result.getCode());
        verify(staffScheduleService).add(entity);
    }
    ...
    void testUpdate_ShouldSetIdAndCallService() {
        ...
        verify(staffScheduleService).update(entity);
    }
    ...
    void testDelete_ShouldCallService() {
        ...
        assertNotNull(result);
        assertEquals(0, result.getCode());
        verify(staffScheduleService).remove(1L);
    }
}