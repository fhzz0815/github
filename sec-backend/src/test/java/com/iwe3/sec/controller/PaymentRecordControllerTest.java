...
/**
 * Unit test for PaymentRecord Controller
 */
...
class PaymentRecordControllerTest {
    ...
    private PaymentRecordController controller;

    @BeforeEach
    void setUp() {
        controller = new PaymentRecordController(paymentRecordService);
    }
    ...
    void testList_ShouldReturnPageResult() {
        ...
        List<PaymentRecordEntity> entityList = Arrays.asList(entity1);
        PageResult<PaymentRecordEntity> pageResult = PageResult.of(1L, 1, entityList);
        when(paymentRecordService.list(any(), anyInt(), anyInt())).thenReturn(pageResult);

        Result<PageResult<PaymentRecordEntity>> result = controller.list(new PaymentRecordEntity(), 1, 10);
        ...
        verify(paymentRecordService).list(any(), eq(1), eq(10));
    }
    ...
    void testGetById_WhenExists_ShouldReturnEntity() {
        ...
        when(paymentRecordService.getById(1L)).thenReturn(mockEntity);

        Result<PaymentRecordEntity> result = controller.getById(1L);
    ...
    void testGetById_WhenNotExists_ShouldReturnNullData() {
        when(paymentRecordService.getById(999L)).thenReturn(null);

        Result<PaymentRecordEntity> result = controller.getById(999L);
    ...
    void testAdd_ShouldCallService() {
        ...
        assertNotNull(result);
        assertEquals(0, result.getCode());
        verify(paymentRecordService).add(entity);
    }
    ...
    void testUpdate_ShouldSetIdAndCallService() {
        ...
        verify(paymentRecordService).update(entity);
    }
    ...
    void testDelete_ShouldCallService() {
        ...
        assertNotNull(result);
        assertEquals(0, result.getCode());
        verify(paymentRecordService).remove(1L);
    }
}